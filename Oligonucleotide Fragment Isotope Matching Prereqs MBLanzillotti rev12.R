...
#In progress fix for combinations of losses and gains in chemical modification, for now use digit modification of average mass if applicable
...
symbol_composition <- 
  tribble(
    ~symbol,    ~C, ~H, ~N, ~O, ~P, ~S, ~Na, ~K,
    "r",        5,  7,  0,  2,  0,  0,  0,   0,#ribose
    "d",        5,  7,  0,  1,  0,  0,  0,   0,#deoxyribose
    "m",        6,  9,  0,  2,  0,  0,  0,   0,#2'O-methylribose
    "+",        6,  7,  0,  2,  0,  0,  0,   0,#Locked 4'-methyl-2' (I think)
    'A',        5,  4,  5,  0,  0,  0,  0,   0,
    "C",        4,  4,  3,  1,  0,  0,  0,   0,
    "G",        5,  4,  5,  1,  0,  0,  0,   0,
    "T",        5,  5,  2,  2,  0,  0,  0,   0,
    "L",        0,  0,  0,  0,  0,  0,  0,   0,#Base loss
    "U",        4,  3,  2,  2,  0,  0,  0,   0,
    "D",        4,  5,  2,  2,  0,  0,  0,   0,#dihydrouridine
    "S",        4,  3,  2,  1,  0,  1,  0,   0,#4-thiouridine
    "Y",       16, 19,  6,  5,  0,  0,  0,   0,#wybutosine
    "*",        0,  1,  0,  1,  1,  1,  0,   0,#thiophosphate backbone
    "p",        0,  1,  0,  2,  1,  0,  0,   0,#phosphate backbone
    "Acetyl",   2,  2,  0,  1,  0,  0,  0,   0,
    "Methyl",   1,  2,  0,  0,  0,  0,  0,   0,
    "Dimethyl", 2,  4,  0,  0,  0,  0,  0,   0, 
) %>% 
...
parse_nucleic_input <- function(input_seq){
  ...
  parsed <- str_extract_all(input_seq, "[rmd\\+]?(\\([12345]?'?[\\+\\-]?[:alnum:]+[.-]?[:alnum:]*\\)){0,5}[ACGTUYSDL]{1}(\\([\\+\\-]?[:alnum:]+[.-]?[:alnum:]+\\))?[p*]?(\\([\\+\\-]?[:alnum:]+[.-]?[:alnum:]+\\))?") %>% 
    as_vector()
  ...
  parsed <- tibble(
    ...
    nitbase = str_extract(parsed, "[ACGTUYSDL]{1}"),
    nitbase_mod = str_extract(parsed, "[ACGTUYSDL]{1}(\\([\\+\\-]?[:alnum:]+[.-]?[:alnum:]+\\))") %>% str_remove("[ACGTUYSDL]"), 
...
fragment_composition <- function(parsed_input, pos, ion_type){
  ...
  if(str_detect(ion_type, "^a$")){
  ...
  } else if(str_detect(ion_type, "^c-B$")){
    
    carrier <- parsed_input %>% 
      filter(
        (position <= pos) & !(position == pos & str_detect(item, "(nitbase)"))
      )
  ...
  } else if(str_detect(ion_type, "^d-B$")){
    
    carrier <- parsed_input %>% 
      filter(
        ((position <= pos) | (position == (pos + 1) & str_detect(item, "(5')"))) & !(position == pos & str_detect(item, "(nitbase)"))
      )
    ...
    carrier <- parsed_input %>%
      filter(
        ((position > pos) & ((position == (pos+1)) & !str_detect(item, "(nitbase)"))) | ((position == pos) & str_detect(item, "(backbone)"))
      )
  ...
  H_adjust <-        c( -1,    -2,   0,  -1,    -1,  +1,    +1,  +1,  -1,    -2,  +1,  -2)
  names(H_adjust) <- c("a", "a-B", "b", "c", "c-B", "d", "d-B", "w", "x", "x-B", "y", "z")
...
generate_frag_formulae <- function(parsed_input, ion_types = c("a", "a-B", "b", "c", "c-B", "d", "d-B", "w", "x", "x-B", "y", "z")){
...
IsoSpec_nominal_dist <- function(IsoSpec_comp){
  ...
  mass_vector <- IsoSpec_to_mass(IsoSpec_comp) + c(0:ceiling(max(nominal_dist$mass) - min(nominal_dist$mass)))*1.002185
  ...
  nominal_dist <- nominal_dist %>% 
    ...
    slice_max(order_by = nuclide, n = 30) %>% 
    ungroup()
...
ID_zcon_frags <- function(fragments, centroids, tolerance = 1e-5){
  ...
  fragments <- fragments %>% 
    mutate(
      ion_type = factor(ion_type, levels = c("w", "a", "a-B", "d", "d-B", "y", "x", "x-B", "z", "b", "c", "c-B"))
    ) %>% 
      ...
      if(isotope_match %>% filter(!is.na(expr_mz)) %>% .$nominal_prob %>% sum() > 0.60){
        ...
        isotope_fit_check <- candidate_fragment %>%
          ...
          mutate(
            ...
            prediction_cutoff = near(predicted_i, intensity, tol = sum(predicted_i)*0.05)#0.05 not as much
          )
        # return(isotope_fit_check)
        if(
          (sum(!isotope_fit_check$prediction_cutoff) <= ceiling(nrow(isotope_fit_check)*0.35)) &&
...
#from a theoretical list in an experimental spectrum. Isotope fitting parameters
...
fetch_profile <- function(spectrum_path, scan_no = 1){
  
  spectrum <- readSpectrum(spectrum_path, scan_no)
...
#retrieves profile data from a designated rawfile

fetch_centroid <- function(spectrum_path, scan_no = 1){
...
#retrieves centroid data from a designated rawfile
